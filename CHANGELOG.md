...
- **`banner` component.** Announcement bar with `default`/`muted` variants and optional `dismissible` button; give it an `id` and the dismissal persists in localStorage with no flash on return visits. Dispatches `banner-dismiss`.
