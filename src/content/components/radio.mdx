...
`RadioGroupItem` also works outside a `RadioGroup` — pass `name` yourself:
</Prose>
...
<div class="flex flex-col gap-2">
  <div class="flex items-center gap-2">
    <RadioGroupItem id="option-one" name="standalone" value="one" />
    <Label for="option-one">Option One</Label>
  </div>
  <div class="flex items-center gap-2">
    <RadioGroupItem id="option-two" name="standalone" value="two" />
    <Label for="option-two">Option Two</Label>
  </div>
</div>
```
...
- `RadioGroupItem` - Individual radio button; also usable standalone with a `name` prop
</Prose>
...
| name | string | - | Group name for standalone use (set automatically inside a RadioGroup) |
