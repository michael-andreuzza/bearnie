---
title: "Input OTP"
description: Accessible one-time password input with copy-paste support.
tagline: OTP verification input
category: "form"
status: "stable"
order: 15
---

import ComponentPreview from "@/components/docs/ComponentPreview.astro";
import CodeBlock from "@/components/docs/CodeBlock.astro";
import Prose from "@/components/docs/Prose.astro";
import InputOTP from "@/components/ui/input-otp/InputOTP.astro";
import InputOTPGroup from "@/components/ui/input-otp/InputOTPGroup.astro";
import InputOTPSlot from "@/components/ui/input-otp/InputOTPSlot.astro";
import InputOTPSeparator from "@/components/ui/input-otp/InputOTPSeparator.astro";

<ComponentPreview>
  <Fragment slot="code">
```html
<InputOTP maxLength={6}>
  <InputOTPGroup>
    <InputOTPSlot index={0} />
    <InputOTPSlot index={1} />
    <InputOTPSlot index={2} />
  </InputOTPGroup>
  <InputOTPSeparator />
  <InputOTPGroup>
    <InputOTPSlot index={3} />
    <InputOTPSlot index={4} />
    <InputOTPSlot index={5} />
  </InputOTPGroup>
</InputOTP>
```
  </Fragment>
  <InputOTP maxLength={6}>
    <InputOTPGroup>
      <InputOTPSlot index={0} />
      <InputOTPSlot index={1} />
      <InputOTPSlot index={2} />
    </InputOTPGroup>
    <InputOTPSeparator />
    <InputOTPGroup>
      <InputOTPSlot index={3} />
      <InputOTPSlot index={4} />
      <InputOTPSlot index={5} />
    </InputOTPGroup>
  </InputOTP>
</ComponentPreview>

<div class="space-y-4">
<Prose>
### Installation
</Prose>

<ComponentPreview component="input-otp" />
</div>

<div class="space-y-4">
<Prose>
### Usage

```ts
import {
  InputOTP,
  InputOTPGroup,
  InputOTPSlot,
  InputOTPSeparator,
} from "@/components/bearnie/input-otp"
```
```html
<InputOTP maxLength={6}>
  <InputOTPGroup>
    <InputOTPSlot index={0} />
    <InputOTPSlot index={1} />
    <InputOTPSlot index={2} />
  </InputOTPGroup>
  <InputOTPSeparator />
  <InputOTPGroup>
    <InputOTPSlot index={3} />
    <InputOTPSlot index={4} />
    <InputOTPSlot index={5} />
  </InputOTPGroup>
</InputOTP>
```

</Prose>
</div>

<div class="space-y-8">
<Prose>
### Pattern

Use the `pattern` prop to define what characters are allowed in the OTP input.
</Prose>

<div class="space-y-4">
<Prose>
#### Digits only (default)
</Prose>

<ComponentPreview>
  <Fragment slot="code">
```html
<InputOTP maxLength={6} pattern="digits">
  <InputOTPGroup>
    <InputOTPSlot index={0} />
    <InputOTPSlot index={1} />
    <InputOTPSlot index={2} />
    <InputOTPSlot index={3} />
    <InputOTPSlot index={4} />
    <InputOTPSlot index={5} />
  </InputOTPGroup>
</InputOTP>
```
  </Fragment>
  <InputOTP maxLength={6} pattern="digits">
    <InputOTPGroup>
      <InputOTPSlot index={0} />
      <InputOTPSlot index={1} />
      <InputOTPSlot index={2} />
      <InputOTPSlot index={3} />
      <InputOTPSlot index={4} />
      <InputOTPSlot index={5} />
    </InputOTPGroup>
  </InputOTP>
</ComponentPreview>
</div>

<div class="space-y-4">
<Prose>
#### Alphanumeric

Use `pattern="alphanumeric"` to accept both letters and numbers.
</Prose>

<ComponentPreview>
  <Fragment slot="code">
```html
<InputOTP maxLength={6} pattern="alphanumeric">
  <InputOTPGroup>
    <InputOTPSlot index={0} />
    <InputOTPSlot index={1} />
    <InputOTPSlot index={2} />
    <InputOTPSlot index={3} />
    <InputOTPSlot index={4} />
    <InputOTPSlot index={5} />
  </InputOTPGroup>
</InputOTP>
```
  </Fragment>
  <InputOTP maxLength={6} pattern="alphanumeric">
    <InputOTPGroup>
      <InputOTPSlot index={0} />
      <InputOTPSlot index={1} />
      <InputOTPSlot index={2} />
      <InputOTPSlot index={3} />
      <InputOTPSlot index={4} />
      <InputOTPSlot index={5} />
    </InputOTPGroup>
  </InputOTP>
</ComponentPreview>
</div>
</div>

<div class="space-y-8">
<Prose>
### Examples
</Prose>

<div class="space-y-4">
<Prose>
#### Separator

Use the `InputOTPSeparator` component to add a separator between input groups.
</Prose>

<ComponentPreview>
  <Fragment slot="code">
```html
<InputOTP maxLength={6}>
  <InputOTPGroup>
    <InputOTPSlot index={0} />
    <InputOTPSlot index={1} />
    <InputOTPSlot index={2} />
  </InputOTPGroup>
  <InputOTPSeparator />
  <InputOTPGroup>
    <InputOTPSlot index={3} />
    <InputOTPSlot index={4} />
    <InputOTPSlot index={5} />
  </InputOTPGroup>
</InputOTP>
```
  </Fragment>
  <InputOTP maxLength={6}>
    <InputOTPGroup>
      <InputOTPSlot index={0} />
      <InputOTPSlot index={1} />
      <InputOTPSlot index={2} />
    </InputOTPGroup>
    <InputOTPSeparator />
    <InputOTPGroup>
      <InputOTPSlot index={3} />
      <InputOTPSlot index={4} />
      <InputOTPSlot index={5} />
    </InputOTPGroup>
  </InputOTP>
</ComponentPreview>
</div>

<div class="space-y-4">
<Prose>
#### Disabled

Use the `disabled` prop to disable the input.
</Prose>

<ComponentPreview>
  <Fragment slot="code">
```html
<InputOTP maxLength={6} disabled>
  <InputOTPGroup>
    <InputOTPSlot index={0} />
    <InputOTPSlot index={1} />
    <InputOTPSlot index={2} />
    <InputOTPSlot index={3} />
    <InputOTPSlot index={4} />
    <InputOTPSlot index={5} />
  </InputOTPGroup>
</InputOTP>
```
  </Fragment>
  <InputOTP maxLength={6} disabled>
    <InputOTPGroup>
      <InputOTPSlot index={0} />
      <InputOTPSlot index={1} />
      <InputOTPSlot index={2} />
      <InputOTPSlot index={3} />
      <InputOTPSlot index={4} />
      <InputOTPSlot index={5} />
    </InputOTPGroup>
  </InputOTP>
</ComponentPreview>
</div>

<div class="space-y-4">
<Prose>
#### Invalid

Use `invalid` on the slots to show an error state.
</Prose>

<ComponentPreview>
  <Fragment slot="code">
```html
<InputOTP maxLength={4}>
  <InputOTPGroup>
    <InputOTPSlot index={0} invalid />
    <InputOTPSlot index={1} invalid />
    <InputOTPSlot index={2} invalid />
    <InputOTPSlot index={3} invalid />
  </InputOTPGroup>
</InputOTP>
```
  </Fragment>
  <InputOTP maxLength={4}>
    <InputOTPGroup>
      <InputOTPSlot index={0} invalid />
      <InputOTPSlot index={1} invalid />
      <InputOTPSlot index={2} invalid />
      <InputOTPSlot index={3} invalid />
    </InputOTPGroup>
  </InputOTP>
</ComponentPreview>
</div>

<div class="space-y-4">
<Prose>
#### Four digits

A common pattern for PIN codes.
</Prose>

<ComponentPreview>
  <Fragment slot="code">
```html
<InputOTP maxLength={4}>
  <InputOTPGroup>
    <InputOTPSlot index={0} />
    <InputOTPSlot index={1} />
    <InputOTPSlot index={2} />
    <InputOTPSlot index={3} />
  </InputOTPGroup>
</InputOTP>
```
  </Fragment>
  <InputOTP maxLength={4}>
    <InputOTPGroup>
      <InputOTPSlot index={0} />
      <InputOTPSlot index={1} />
      <InputOTPSlot index={2} />
      <InputOTPSlot index={3} />
    </InputOTPGroup>
  </InputOTP>
</ComponentPreview>
</div>

<div class="space-y-4">
<Prose>
#### Custom separator

You can pass custom content to the separator.
</Prose>

<ComponentPreview>
  <Fragment slot="code">
```html
<InputOTP maxLength={6}>
  <InputOTPGroup>
    <InputOTPSlot index={0} />
    <InputOTPSlot index={1} />
    <InputOTPSlot index={2} />
  </InputOTPGroup>
  <InputOTPSeparator>
    <span class="text-muted-foreground">-</span>
  </InputOTPSeparator>
  <InputOTPGroup>
    <InputOTPSlot index={3} />
    <InputOTPSlot index={4} />
    <InputOTPSlot index={5} />
  </InputOTPGroup>
</InputOTP>
```
  </Fragment>
  <InputOTP maxLength={6}>
    <InputOTPGroup>
      <InputOTPSlot index={0} />
      <InputOTPSlot index={1} />
      <InputOTPSlot index={2} />
    </InputOTPGroup>
    <InputOTPSeparator>
      <span class="text-muted-foreground">-</span>
    </InputOTPSeparator>
    <InputOTPGroup>
      <InputOTPSlot index={3} />
      <InputOTPSlot index={4} />
      <InputOTPSlot index={5} />
    </InputOTPGroup>
  </InputOTP>
</ComponentPreview>
</div>
</div>

<Prose>
### Anatomy

- `InputOTP` - The main container that manages OTP state
- `InputOTPGroup` - Groups slots together with connected styling
- `InputOTPSlot` - Individual input slot for each digit
- `InputOTPSeparator` - Visual separator between groups
</Prose>

<Prose>
### Props

#### InputOTP

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| maxLength | number | required | Maximum length of the OTP (number of slots) |
| pattern | digits \| alphanumeric \| string | digits | Pattern to validate input |
| disabled | boolean | false | Disables all slots |
| defaultValue | string | - | Initial value |
| name | string | - | Name attribute for form submission |
| class | string | - | Additional CSS classes |

#### InputOTPGroup

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| class | string | - | Additional CSS classes |

#### InputOTPSlot

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| index | number | required | Index of the slot (0-based) |
| invalid | boolean | false | Shows error state |
| class | string | - | Additional CSS classes |

#### InputOTPSeparator

| Prop | Type | Default | Description |
|------|------|---------|-------------|
| class | string | - | Additional CSS classes |

### Events

The root element dispatches bubbling CustomEvents as the user types — no polling required:

```js
const otp = document.querySelector("[data-input-otp]");

otp.addEventListener("otp-change", (e) => {
  console.log(e.detail.value); // current value, e.g. "12"
});

otp.addEventListener("otp-complete", (e) => {
  submitCode(e.detail.value); // fires when all slots are filled
});
```
</Prose>
