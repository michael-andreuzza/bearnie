---
title: Introduction
description: 50+ accessible, open source components for Astro and Tailwind CSS. One CLI command copies the source into your project — customize, extend, and own the code.
...
If you've used shadcn/ui, this is that workflow — built for Astro instead of React. Browse the components, run one command, and the source is yours.
...
- **MCP**: Let AI assistants add components for you, including batch installs via `add_components`.
- **Registry**: A versioned flat-file schema that defines components and their dependencies.
</Prose>
...
Import directly from a component file:
...
Or use the barrel export after running `bearnie add barrel`:

```astro
---
import { Button } from '@/components/bearnie';
---
